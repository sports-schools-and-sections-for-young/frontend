  ...
  return (
    <div className={styles.slider}>
      <div className={styles.containerSlider}>
        <div className={styles.containerSwipe}>
          <button 
            className={`${styles.buttonLeft} ${currentSlide === 0 ? styles.hidden : ''}`} 
            type="button" 
            onClick={prevSlide}
            aria-label="Предыдущий слайд"
          />
          <div className={styles.containerText}>
            <p className={styles.kindOfSport}>{sportsData[currentSlide].name}</p>
            <p className={styles.textSlider}>{sportsData[currentSlide].text}</p>
            <div className={styles.containerButton}>
            <Button 
              ...
              Записаться 
              <Icon 
                type={IconTypes.RIGHT_ICON} />
            </Button>
              <span className={styles.numberSlider}>
                <span className={styles.leadingNumbers}>{formatNumber(currentSlide + 1)}
                </span>/{formatNumber(sportsData.length)}
              </span>
            </div>
          </div>
          <img className={styles.imgSlider} src={sportsData[currentSlide].imageSrc} alt={sportsData[currentSlide].name} />
          <button 
            className={`${styles.buttonRight} ${currentSlide === sportsData.length - 1 ? styles.hidden : ''}`} 
            type="button" 
            onClick={nextSlide}
