...
const YandexMap: FC<YandexMapProps> = ({ center }) => {
  const objectManager = useRef<ObjectManagerProps>(null);
  ...
  return (
    <>
      <YMaps>
        <div className={styles.mapWrapper}>
          ...
          >
            ...
            {/*        properties={{ balloonContent: `<div class="balloon">${point.content}</div>`}} */}
            {/*        options={{iconLayout: 'default#image', iconImageHref: icon, iconImageSize: [40, 40], iconImageOffset: [-20, -40], hasBalloon: true}} */}
      ...
      <div className={styles.objects}>
        {visible.length
          ? visible.map((obj) => (
              <div className={styles.object}>{`Объект номер: ${obj.id}`}</div>
            ))
          : null}
      </div>
    </>
